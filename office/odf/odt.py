...
class ArquivoODT:
    def __init__(self):
        ...
        self._result = {}
        self._zipdata = None
        self._modeloSourceFileURL = ""
    def setModeloSourceFileURL(self, *zipSourceFile):
        self._modeloSourceFileURL=path.join(*zipSourceFile)
    ...
    def extrair(self):
        ...
        self._tmpDirURL=path.join(tempfile.gettempdir(), "tmpODT", tmpDir)
        try:
            os.mkdir( path.join(tempfile.gettempdir(), "tmpODT") )
        except OSError as error:
            print("Se arquivo já foi criado ignore, caso contrario confira o acesso da pasta 'tmpODT'")
        ...
        self._zipdata = zipfile.ZipFile(self._modeloSourceFileURL)
    ...
    def comprimir(self):
        ...
        zipOutFileURL=path.join(tempfile.gettempdir(), "tmpODT", "prontos", zipOutFile)
        try:
            os.mkdir( path.join(tempfile.gettempdir(), "prontos", "prontos") )
        except OSError as error:
    ...
    def getStream(self):
        ...
        with zipfile.ZipFile(archive, 'w') as outzip:
            zipinfos = self._zipdata.infolist()
            for zipinfo in zipinfos:
                fileName=zipinfo.filename # The name and path as stored in the archive
                fileURL=path.join(self._tmpDirURL, fileName)# The actual name and path
